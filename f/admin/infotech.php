<?php
    ...
    try {
        if(isset($_GET['search'])) {
            ...
            $stmt = $pdo->prepare("SELECT * FROM `studies` WHERE (dept = 'Information Technology') AND (LOWER(title) REGEXP :search OR LOWER(keywords) REGEXP :search) LIMIT :offset, :limit");
            $stmt->bindParam(':search', $search, PDO::PARAM_STR);
            $stmt->bindParam(':offset', $offset, PDO::PARAM_INT);
            $stmt->bindParam(':limit', $studiesPerPage, PDO::PARAM_INT);
    
            // Fetch total number of studies for search results
            $totalStmt = $pdo->prepare("SELECT COUNT(*) AS total FROM `studies` WHERE dept = 'Information Technology' AND (LOWER(title) REGEXP :search OR LOWER(keywords) REGEXP :search)");
            $totalStmt->bindValue(':search', $search, PDO::PARAM_STR);
        } else {
            $stmt = $pdo->prepare("SELECT * FROM `studies` WHERE dept = 'Information Technology' LIMIT :offset, :limit");
            $stmt->bindParam(':offset', $offset, PDO::PARAM_INT);
            $stmt->bindParam(':limit', $studiesPerPage, PDO::PARAM_INT);
    
            // Fetch total number of all studies for the Information Technology department
            $totalStmt = $pdo->prepare("SELECT COUNT(*) AS total FROM `studies` WHERE dept = 'Information Technology'");
        }
