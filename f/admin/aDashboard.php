<?php
    ...
    try {
        if (isset($_GET['search'])) {
            $search = '%' . $_GET['search'] . '%';
            $stmt = $pdo->prepare("SELECT * FROM `studies` WHERE title LIKE :search LIMIT :limit OFFSET :offset");
        ...
        if (isset($_GET['search'])) {
            $totalSearchStmt = $pdo->prepare("SELECT COUNT(*) AS total FROM `studies` WHERE title LIKE :search");
            $totalSearchStmt->bindValue(':search', $search, PDO::PARAM_STR);
            $totalSearchStmt->execute();
            $totalSearchResults = $totalSearchStmt->fetch(PDO::FETCH_ASSOC)['total'];
        }
    } catch (PDOException $e) {
        echo $e->getMessage();
    }
